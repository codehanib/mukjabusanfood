...
<header class="site-header">
	...
	<div class="top-menu">
		<div class="top-menu-inner">
			...
			<sec:authorize access="isAnonymous()">
				...
				<a href="/reservation/guestForm">비회원 예약확인</a>
			</sec:authorize>
			...
			<sec:authorize access="hasRole('OWNER')">
				<a href="/restaurant/ownerpage">식당관리</a>
			</sec:authorize>
	...
	<div class="site-header-inner">
		<!-- 로고 -->
		<a href="/main" class="site-logo"> <img src="/images/logo.png"
			alt="mukja 로고">
		</a>
		<!-- 검색창 영역 -->
		<div class="site-search-area">
			...
			<form action="/restaurant/search" method="get" class="search-form"
				...
				<input type="text" name="keyword" id="keyword"
					placeholder="식당, 메뉴, 지역 검색" autocomplete="off">
				<button type="submit">검색</button>
				<!-- 자동완성 목록 -->
				<div id="suggestions"
					style="display: none; position: absolute; top: 100%; left: 0; background: white; border: 1px solid #cccccc; width: 170px; z-index: 10;">
				</div>
			</form>
			<div class="popular-box">
				<div class="popular-top" id="popularToggle">
					...
					<c:if test="${not empty popularSearchList}">
						<span class="popular-first"> 1.
							${popularSearchList[0].ms_word} </span>
					</c:if>
					<span class="popular-arrow">▼</span>
				</div>
				<div class="popular-list" id="popularList">
					<c:forEach var="search" items="${popularSearchList}"
						varStatus="status">
						<a href="/restaurant/search?keyword=${search.ms_word}">
							${status.index + 1}. ${search.ms_word} </a>
					</c:forEach>
				</div>
			</div>
		</div>
